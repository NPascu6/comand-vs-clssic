...
- **`Program.cs`** — no DI container on purpose: newing everything up by hand
  makes the wiring count visible. Ten construction steps (upstream bundle,
  mapper, four repositories, config factory plus load, validator factory plus
  registration, service, controller) run before a single request can be
  handled. `CultureInfo.InvariantCulture` is pinned so `N0` amounts render the
  same on every machine.
- **`Atlas.Classic.NTier.csproj`** — `appetite.config.json` must be marked as
  `Content` with `CopyToOutputDirectory`, or `AppetiteConfigFactory` throws at
  runtime: one more moving part the feature depends on.
- **`Configuration/`** — appetite limits are policy, so the classic stack
  externalises them: the JSON file, a typed options model and a loader factory,
  three artefacts for what is conceptually a small lookup table. (The other
  samples read appetite from the upstream client; this one uses config on
  purpose, to exhibit the layer.) The factory reads enums by name, so a typo in
  the file fails deserialization instead of silently mapping to enum 0, matches
  property names case-insensitively, and treats a missing or empty file as a
  hard configuration error.
- **`Controllers/CommitmentController.cs`** — even this thin controller does
  four jobs: validate, map in, delegate, map out. A DTO that fails
  DataAnnotations is shaped into a 400 and never reaches the service, so the
  homegrown validator's second pass does not run for bad DTOs.
- **`Mapping/CommitmentMapper.cs`** — by the time `ToRequest` runs the
  `[Required]` strings are non-null in practice, but the DTO type still says
  `string?`, so every field is coalesced to satisfy the non-nullable domain
  record: the nullable-shape mismatch tax between two models. Get one
  assignment wrong in any of the six mappings and a field silently travels as
  `default(T)`.
- **`Repositories/Entities/ExposureEntity.cs`** — the private copy of the
  bucket-key convention means that if `Buckets.Key` ever changes format, the
  entity silently disagrees and the appetite rule reads the wrong bucket. The
  entity boundary bought isolation at the cost of a hidden second source of
  truth.
- **`Services/CommitmentService.cs`** — rule 2 adds an error and returns
  (unlike the ValidatorFactory sample, which throws) because the later rules
  dereference the fund; from rule 3 on, errors accumulate. Booking is not a
  real write; it only stamps an id.
- **`Validation/ValidatorFactory.cs`** — the cast in `GetValidator<T>` is
  unchecked by the compiler: register the wrong instance under a type and you
  get an `InvalidCastException` at runtime, not a build error. The sync
  `IValidator<T>` suffices here only because the service does the async
  repository work before the validator ever runs — the same tension as the
  ValidatorFactory sample, relocated.
...
**Bottom line:** one operation, **29 `.cs` files / ~800 LOC across ~8 layers**
