...
public sealed class ExposureRepository : IExposureRepository
{
    ...
    public async Task<ExposureEntity> GetByFundAsync(string fundId, CancellationToken ct = default)
    {
        ExposureSnapshot snapshot = await _client.GetExposureAsync(fundId, ct);
