...
public sealed class CoInvestmentRepository : ICoInvestmentRepository
{
    ...
    public async Task<CoInvestmentEntity?> GetByIdAsync(string coInvestmentId, CancellationToken cancellationToken = default)
    {
        CoInvestmentNode? node = await _client.GetNodeAsync(coInvestmentId, cancellationToken);
