...
One CI gate plus one pipeline per deployable unit: the web app, storybook and
the API. The gate proves that every unit builds, tests and lints; the per-unit
pipelines deploy — and only the unit whose sources changed. Each deploy pipeline
also owns exactly one Terraform stack (`infra/stacks/<stack>`) and applies it
before it deploys, so a unit's infrastructure and its code ship together and
nothing else has to run first.

Design-token changes are gated and reviewed like code: see [docs/design-tokens-governance.md](../docs/design-tokens-governance.md).

## Pipelines

| Pipeline | Runs on | Stack | What it does |
| --- | --- | --- | --- |
| `/azure-pipelines.yml` | pull requests, pushes to `main` | — | The gate, four parallel jobs. **Web**: `pnpm install --frozen-lockfile`, design-tokens check, the package builds, `typecheck`, the proof-of-concept build, `build:storybook`; publishes `web-app` and `storybook` artefacts. **Api**: `dotnet build`, `dotnet test` (trx results published), `dotnet pack` → `nuget` artefact. **Lint**: `deno lint` + `deno test -A` (the same gate as `.github/workflows/deno.yml`). **Infra**: `terraform fmt -check` + `validate` for each of the four stacks (a matrix). Nothing deploys. |
| `deploy-web-app.yml` | `main`; paths `web/apps/designer-poc`, `web/packages`, `web/build`, `infra/stacks/web-app`, `infra/modules`, `infra/env` | `web-app` | `pnpm build:packages` + `pnpm --filter @atlas/designer-poc build` → artefact `web-app`. Per environment: `terraform apply` of the `web-app` stack, then the artefact goes to Static Web App **app** with the deployment token the stack outputs (`api_key`). `config.json` — `apiBaseUrl` — is written into the artefact per environment (see below). Dev, then prod. |
...
| `publish-packages.yml` | `main` and `v*` tags; paths `web/packages`, `src` | — | Stamps a version, `pnpm publish -r` every workspace package to the npm feed, `dotnet pack` + `dotnet nuget push` `Atlas.Functional.Commands` and `Atlas.Upstream.Contracts` to the NuGet feed. |
| `infra.yml` | pull requests: `infra/**`; `main`: `infra/stacks/foundation`, `infra/modules`, `infra/env` | `foundation` | Pull request: `terraform plan` of `foundation`, `api`, `web-app` and `storybook` against dev, so a shared module or tfvars change is reviewed for every stack. `main`: apply dev → plan prod → apply prod. |
...
A change under `web/apps/designer-poc/` or `web/packages/` runs the web-app deploy; a
change under `web/packages/core/` or `web/packages/design-tokens/` runs the
storybook deploy with it; a change under `src/` runs the API deploy and the
package publish; a change under `infra/stacks/api/` runs only the API deploy.
Only what is shared fans out: the Terraform modules and tfvars
(`infra/modules/`, `infra/env/`) to every pipeline that applies a stack. Each pipeline builds its unit from scratch in its own
job, so nothing is shared between them at run time except the variable groups. Prod
...
exactly one pipeline. Stacks never read each other's state inside Terraform:
they share the naming convention (`infra/modules/naming`) and look shared
resources up by name.
...
3. `deploy-web-app.yml` and `deploy-storybook.yml`, in either order — each
   creates its Static Web App and deploys. The web app's `config.json` still
   points at a placeholder until step 5, which runs it again.
...
`@atlas/platform`). The deploy template writes
`{ "apiBaseUrl": "<API_BASE_URL>" }` into the downloaded artefact before
uploading it, so the same build goes to dev and prod. The API takes its settings as environment variables on the
Container App (`ASPNETCORE_URLS`, `Cors__Origins__0..n`, `I18n__Folder`) — those are
...
| `terraform.yml` | Parameters `stack` (`foundation` \| `api` \| `web-app` \| `storybook`), `env` (`dev` \| `prod`), `action` (`validate` \| `plan` \| `apply` \| `output`), `stateName` (the state blob when it differs from the stack), `vars` (extra `-var` arguments) and `outputs` (a list of `{ name, variable, secret }`). Works in `infra/stacks/<stack>` with `-var-file ../../env/<env>.tfvars` and state key `<env>/<stateName or stack>.tfstate`. `validate` needs no Azure access; `plan`/`apply`/`output` run inside `AzureCLI@2` on `atlas-azure`, configure the azurerm backend from the `TF_STATE_*` variables, then copy each requested output (`terraform output -raw <name>`) into a variable of the job, secret when asked. `output` changes nothing and only exports; a state that was never applied exports empty values with a warning, where `plan`/`apply` fail on an empty output. |
| `deploy-static-web-app.yml` | Download an artefact, optionally write `config.json` (`apiBaseUrl`), `AzureStaticWebApp@0` with `skip_app_build` and the deployment token passed in. |
...
deployment job that targets it waits there: `Deploy_Prod` in every deploy
pipeline (web-app, storybook, api) and `Apply_Prod` in
`infra.yml` (review the `Plan_Prod` log first).
...
(six pipelines: the gate, `infra`, `publish-packages` and the three unit
deploys). For a GitHub repository the `pr:`
triggers in `azure-pipelines.yml` and `infra.yml` are honoured as written. For an Azure
...
pnpm -r --filter './packages/**' run build && pnpm typecheck && pnpm --filter @atlas/designer-poc build && pnpm build:storybook
