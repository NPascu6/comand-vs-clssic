...
Every deployable (web app, storybook, API) has to be developed,
deployed and rolled back on its own. If all of them sat in one root module,
every apply would plan the whole environment, a broken change in one unit would block the
...
committed `.terraform.lock.hcl`, and one state blob of its own.
...
stack declares the whole set**: `variables.tf` is identical in all five stacks
...
No stack reads another stack's outputs, inside Terraform or in a pipeline.
...
2. **`web-app`** and **`storybook`**, in either order or in parallel: run
   their deploy pipelines. Each applies its own stack, then deploys the build
   with the token it just read.
...
`<env>/<stack>.tfstate` — or `<env>/<stateName>.tfstate` when the caller
names the state — passes `-var-file ../../env/<env>.tfvars`, and exposes validate / plan /
apply / output actions. Plan, apply and output run inside `AzureCLI@2` on the
`atlas-azure` service connection, which hands its identity to the provider
and the backend through `ARM_*` variables; `output` only reads a state and
exports the requested outputs (empty, with a warning, for a state that does
not exist yet).

| Pipeline | Terraform it runs |
|---|---|
| `azure-pipelines.yml` (CI gate, PRs and `main`) | `terraform fmt -check` + `init -backend=false` + `validate` for the four stacks (a matrix); no Azure access |
...
| `deploy-web-app.yml` | Build → per environment: apply `web-app`, read `terraform output -raw api_key` into a secret pipeline variable, write `config.json`, deploy `dist/` with it |
