...
| 15–25 | **The functional core** — owned core, a rule, errors-as-values, traceability; two styles one core; "convention, not a cage". | §3 + run `dotnet run --project src/Atlas.Functional.Commands.Demo` → **all errors in one pass + the decision-trace JSON** |
...
- **"Isn't this just MediatR / CQRS?"** — Same command-handler *shape* Microsoft endorses, but ~300 lines we own — no library, async-native, with a built-in decision trace.
...
dotnet run   --project src/Atlas.Functional.Commands.Demo      # functional: all errors + trace
