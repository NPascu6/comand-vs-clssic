...
## 4. The core you own (`src/Atlas.Functional.Commands/Core/`, ~470 lines, zero third-party deps)
...
| `Spec<T>` | A declarative, FluentValidation-style builder (`RuleFor(x => x.Field).NotEmpty()...`, `.When`, `MustAsync`, `Include`) that produces `Rule<T>` — same machinery, no library. |
...
- **Learning curve** — `Result`/rules is new to an OO team → but it's plain C#; the core is
  ~470 lines read in an afternoon.
...
dotnet run   --project src/Atlas.Functional.Commands        # all errors in one pass + the trace JSON
