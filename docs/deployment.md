...
|---|---|---|---|---|---|---|
| 1 | proof of concept | `web/apps/designer-poc` | `pnpm --filter @atlas/designer-poc build` (Vite) | `web/apps/designer-poc/dist/` — `index.html`, hashed assets — + `dist/config.json` written per environment | Azure Static Web App **app** (`<prefix>-<env>-app`) | `infra/stacks/web-app` |
...
| 4 | npm packages | `web/packages/*` | `pnpm build:packages` (`tsc -p tsconfig.build.json` per package) | `dist/` per package (ESM + `.d.ts`) | Azure Artifacts npm feed | — |
...
- **The web app is a static site.** Vite writes `index.html`, hashed assets and everything
  in `web/apps/designer-poc/public/` — `config.json` and `staticwebapp.config.json` — into `dist/`.
...
- **Storybook is a second kind of static site** with no runtime configuration at all.
...
- **The npm packages stay source-linked in development** (`main` → `./src/index.ts`,
  ...
  sources out of the tarball. `@atlas/designer-poc` is `private` and is never published.
...
### A web app: `/config.json`

The proof of concept is static — it reads the tokens it ships with and calls nothing — so it
needs no runtime configuration. An app that does talk to the API awaits `loadRuntimeConfig()`
from `@atlas/platform` (`web/packages/platform/src/runtimeConfig.ts`) before rendering
anything. It fetches `/config.json` with `cache: 'no-store'` and expects

```json
{ "apiBaseUrl": "https://<api host>/api" }
```

A missing, unreadable or malformed file yields the default —
`http://localhost:5179/api` — and never throws. Locally that file is
`web/apps/designer-poc/public/config.json`, which Vite copies into `dist/`; on deploy,
`.azure-pipelines/templates/deploy-static-web-app.yml` overwrites `dist/config.json` with
the environment's `API_BASE_URL` before uploading (section 3). Keep the `/api` suffix: a
caller appends `/design-tokens/<mode>` and `/i18n/…` to it.
...
| `Cors__Origins__0..n` | the `web_origins` list in `infra/env/<env>.tfvars` — the web-app stack's `url` | `http://localhost:5174`, `http://localhost:4173` from `appsettings.Development.json` | browser origins allowed to call the API |
...
|---|---|---|---|
| `azure-pipelines.yml` | PR + `main` | none | **CI** — four parallel jobs: Web (design-tokens check, the package builds, `typecheck`, the proof-of-concept build, `build:storybook`), Api (`dotnet build`, `test`, `pack`), Lint (`deno lint`, `deno test -A`), Infra (`terraform fmt -check` + `validate`, a matrix over the four stacks, no Azure access). Publishes the `web-app`, `storybook` and `nuget` artefacts. Deploys nothing. |
| `.azure-pipelines/deploy-web-app.yml` | `main` | `web/apps/designer-poc/**`, `web/packages/**`, `web/build/**`, `infra/stacks/web-app/**`, `infra/modules/**`, `infra/env/**` | Build → Deploy_Dev → Deploy_Prod. Each deploy stage applies the `web-app` stack, reads its `api_key`, writes `config.json` and deploys `dist/` |
...
| `.azure-pipelines/publish-packages.yml` | `main`, tags `v*` | `web/packages/**`, `src/**` | Publish — jobs Npm (`pnpm publish -r`) and NuGet (`dotnet pack` + `dotnet nuget push`) |
...
Path triggers keep the units independent. A push under `web/apps/designer-poc/` or
`web/packages/` runs the web-app deploy, and one under `web/packages/core/` or
`web/packages/design-tokens/` runs storybook's with it; a push under `src/` runs the API
deploy and the package publish; a push under `infra/stacks/foundation/` runs `infra.yml`
alone. Only what is shared fans out: the Terraform modules and tfvars (`infra/modules/`,
`infra/env/`) to every pipeline that applies a stack.
...
- **One state per stack.** Each stack keeps its own blob in the state container,
  ...
  parameters, or from `stateName` when the caller names the state.
- **Coupling by name, not by state.** `foundation` owns what is shared by definition; the
  ...
  `azurerm_application_insights` and `azurerm_storage_account`; the sites read the
  resource group. No `terraform_remote_state`, so no stack depends on another stack's state
  file or output layout. The one value that crosses stacks inside Terraform — the web app's
  origin, for CORS — travels through `web_origins` in `env/<env>.tfvars`.
- **Apply order.** `foundation` first, then `web-app` and `storybook` in either order,
  then `api`, which needs the registry and a pushed image (first bring-up, below). After
  that any order: each pipeline applies its own stack on every run and a no-op apply is
  cheap.
- **A pipeline applies only its own stack.** `deploy-web-app` and `deploy-storybook` run
  `terraform apply` on their stack, then
  ...
  only. No stack reads another stack's state, in Terraform or in a pipeline.
...
2. Create the service connection `atlas-azure` and the environments `atlas-dev` and
   `atlas-prod` (section 6); register the twelve pipelines (the gate, `infra`,
   `publish-packages` and the three unit deploys).
...
2. **`web-app`** and **`storybook`**, in either order — run their deploy pipelines. Each
   applies its own stack, then deploys the build with the token it just read. Until step 5
   the web app's `config.json` points at a placeholder API.
...
pnpm build:packages           # tsc every package → its dist/ (ESM + .d.ts)
pnpm --filter @atlas/designer-poc build   # Vite → apps/designer-poc/dist/: index.html, assets (config.json comes from public/)
pnpm build:storybook          # → packages/core/storybook-static/
pnpm preview                  # serve apps/designer-poc/dist on http://localhost:4173
...
`web/apps/designer-poc/dist/config.json` before `pnpm preview` — the same thing the deploy
template does. The composition preview writes that file itself, pointed at the `dotnet run`
API on `:5179`; Mock needs nothing, and the Live toggle from `:4180` works once that origin
is in the API's CORS list (`appsettings.Development.json` lists `:5173` and `:4173`). Plan
and apply need Azure credentials: `az login`, `export
...
connection. One token per site per environment — the web app and storybook, four in total —
and neither is stored anywhere: each deploy reads its own stack's `api_key` output at deploy
time.
...
2. Run `deploy-web-app.yml` (or `deploy-storybook.yml`) again —
   re-run its deploy stage or queue a new run on `main`. The stage re-applies the stack,
   which refreshes `api_key` from the resource, reads the new value and deploys with it;
   check the stage goes green.

Nothing to paste. To read the value yourself: in `infra/stacks/web-app` (or `storybook`),
`init` against the state (section 8), `terraform apply -var-file=../../env/<env>.tfvars` — a no-op apart
from the refresh — then `terraform output -raw api_key`.

### Ship or roll back the web app

The normal path is a merge to `main` under `web/apps/designer-poc/**` or `web/packages/**`:
`deploy-web-app.yml` builds, applies the `web-app` stack (a no-op once the site exists),
deploys to dev, waits for the `atlas-prod` approval and deploys to prod. A rollback is that
pipeline's earlier run: re-run its deploy stage and the `dist/` built from that commit goes
back up, with `config.json` rewritten for the environment.
...
3. Check `curl https://<api fqdn>/api/i18n/locales` lists it — the store re-reads the disk
   ...
   disable it or change its fallback, `PUT /api/i18n/config`. The first edit through the API records the file as version 1
